...
func printOneliner1() {
	...
	txt := textClr.Sprintf("              Modified by (") + handleClr.Sprintf("TELEGRAM @NEXSOWNER") + textClr.Sprintf(")") + spc + textClr.Sprintf("Version ") + versionClr.Sprintf("%s", VERSION)
	fmt.Fprintf(color.Output, "%s", txt)
}
...
func printOneliner2() {
	...
	txt := red.Sprintf("                   Don't be evil") + white.Sprintf(" - ") + textClr.Sprintf("Office")
	fmt.Fprintf(color.Output, "%s", txt)
}
